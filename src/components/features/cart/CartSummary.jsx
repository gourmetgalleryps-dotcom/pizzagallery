 ...
 * Displays price breakdown, promo code input, and WhatsApp order button
 */
export default function CartSummary({ customerInfo, onCustomerInfoSubmit, onCustomerInfoCancel }) {
  ...
  const handleWhatsAppOrder = async () => {
    ...
    try {
      ...
      const [whatsappResult, sheetsResult] = await Promise.allSettled([
        // Open WhatsApp
        new Promise((resolve) => {
          if (whatsAppData.url) {
        ...
        sendToGoogleSheets(orderData, GOOGLE_SHEETS_WEB_APP_URL),
      ]);
  ...
  // Show customer info form if not provided
  if (!customerInfo) {
    return (
      <CustomerInfoForm
        onSubmit={onCustomerInfoSubmit}
        onCancel={onCustomerInfoCancel}
      />
    );
  }

  return (
    <div className="flex-shrink-0 bg-white border-t border-gray-100 p-6 pb-8 space-y-4 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] z-20">
      {/* Promo Code */}
      <div>
